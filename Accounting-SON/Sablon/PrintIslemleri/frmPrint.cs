...
namespace Accounting.PrintIslemleri
{
    public partial class frmPrint : Form
    {
        ...
        private void Alis()
        {
            AlSat.frmAlis als =Application.OpenForms["frmAlis"] as AlSat.frmAlis;   //Form.ActiveForm(); burda çalışmıyo ama aynısı
            pAlis cr = new pAlis();
            var srg = (from s in db.vwAlis
                       where s.PurNo == int.Parse(als.txtAlisNo.Text)
                       select s).ToList();   /*   bilgiyi vwAlis'tan çekiyoruz*/
